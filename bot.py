#  7212098902:AAFf6eqK7onWtG20UpLICecSyyG_Z1v7Ku4
import telebot
...
API_TOKEN = '7212098902:AAFf6eqK7onWtG20UpLICecSyyG_Z1v7Ku4'
...
main_menu.add(btn_get_task, btn_redo, btn_add, btn_show_progress, btn_delete_progress)
...
with open('users_data\\users.json', 'r+', encoding='utf-8') as f:
    try:
        ...
        user = {int(k): obj_creation(v) for k, v in json_data.items()}
...
# => creating new user profile
    user[message.chat.id] = User(message.chat.id) 
...
# => saving new user
    ...
    bot.send_message(message.chat.id, 'Привіт! Я допоможу тобі підготуватися до НМТ з фізики. Для детального ознайомлення з кожною функцією: /help', reply_markup=main_menu)
...
def get_task(message):
    ...
    btn_anouther_task = types.InlineKeyboardButton('Змінити задачу', callback_data='anouther_task')

    markup = types.InlineKeyboardMarkup()
    markup.add(btn_back, btn_anouther_task)

    is_text, title, mes = user[message.chat.id].get_task(False)

# => message sending command depends of question format: image or text
    bot.send_message(message.chat.id, title, reply_markup=types.ReplyKeyboardRemove())
    if is_text:
...
def show_progress(message):
    ...
    bot.send_message(message.chat.id, user[message.chat.id].show_progress())
...
def add_theme(message):
    ...
    with open('users_data/users.json', 'r', encoding='utf-8') as f:
                ...
                file[str(message.chat.id)]['is_add'] = True
    ...
    bot.send_message(message.chat.id, 'Напишіть номери тем, які ви вже вивчили:', reply_markup=types.ReplyKeyboardRemove())
...
def delete_progress(message):
    ...
    bot.send_message(message.chat.id, 'Всі ваші правильні відповіді буде видалено', reply_markup=types.ReplyKeyboardRemove())
    bot.send_message(message.chat.id, 'Ви впевнені, що хочете видалити свій прогрес?', reply_markup=markup)
...
def redo(message): 
    ...
    is_text, title, mes = user[message.chat.id].get_task(True)
...
def mes_proc(message):
    ...
    with open('users_data/users.json', 'r', encoding='utf-8') as f:
                file = json.loads(f.read())
                is_add = file[str(message.chat.id)]['is_add']
                current_q = file[str(message.chat.id)]['current_q']

# => end of themes adding function
    if is_add:
        print('add theme')
        t_list = message.text.split()
        user[message.chat.id].add_theme(t_list)   
        ...
        with open('users_data/users.json', 'r', encoding='utf-8') as f:
                file = json.loads(f.read())
                file[str(message.chat.id)]['is_add'] = False
...
# => cheking answers after giving task
    if current_q:
        ...
        mes = user[message.chat.id].check_ans(message.text.strip().replace(',', '.'))
...
def callback_proc(call):
    ...
    # => getting new task
        is_text, title, mes = user[call.message.chat.id].get_task(False)
    ...
    # => saving that bot is not waiting for answer or theme adding
        user[call.message.chat.id].back()
    # => deleting inline keyboard from task message
        bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=None)
    # => adding replay keyboard
        bot.send_message(call.message.chat.id, 'Повертаюся...', reply_markup=main_menu)
        
    
    if call.data == 'delete_data':
        user[call.message.chat.id].delete_progress()
        bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=None)
        bot.send_message(call.message.chat.id, 'Прогрес скинуто', reply_markup=main_menu)
